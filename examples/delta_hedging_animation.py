...
def parse_args():
    ...
    parser.add_argument(
        "--hedge_freq", type=int, default=21, help="Hedge rebalancing frequency (steps)"
    )
...
def main():
    ...
    ax1.axhline(
        args.strike, color="red", linestyle="--", alpha=0.5, label=f"Strike = {args.strike}"
    )
    ...
    price_lines = [
        ax1.plot([], [], color=colors[i], linewidth=1.5, alpha=0.8)[0] for i in range(args.n_paths)
    ]
    delta_lines = [
        ax2.plot([], [], color=colors[i], linewidth=1.5, alpha=0.8)[0] for i in range(args.n_paths)
    ]
    pnl_lines = [
        ax3.plot([], [], color=colors[i], linewidth=1.5, alpha=0.8, label=f"Path {i+1}")[0]
        for i in range(args.n_paths)
    ]

    # Current position markers
    price_dots = [
        ax1.plot([], [], "o", color=colors[i], markersize=8)[0] for i in range(args.n_paths)
    ]
    delta_dots = [
        ax2.plot([], [], "o", color=colors[i], markersize=8)[0] for i in range(args.n_paths)
    ]
    pnl_dots = [
        ax3.plot([], [], "o", color=colors[i], markersize=8)[0] for i in range(args.n_paths)
    ]
    ...
    fig.suptitle(
        f"Delta Hedging Simulation: S₀={args.spot}, K={args.strike}, "
        f"σ={args.volatility:.0%}, r={args.rate:.1%}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    def init():
        ...
        time_text.set_text("")
        return (
            price_lines + delta_lines + pnl_lines + price_dots + delta_dots + pnl_dots + [time_text]
        )
    ...
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=args.n_steps, interval=1000 // args.fps, blit=True
    )
